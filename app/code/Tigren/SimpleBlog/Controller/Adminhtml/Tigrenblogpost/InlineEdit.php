        ...
        if ($this->getRequest()->getParam('isAjax')) {
            ...
            if (!count($postItems)) {
                ...
                foreach (array_keys($postItems) as $modelid) {
                    /** @var \Tigren\SimpleBlog\Model\TigrenBlogPost $model */
                    $model = $this->_objectManager->create(\Tigren\SimpleBlog\Model\TigrenBlogPost::class)->load($modelid);
                    try {
                        $model->setData(array_merge($model->getData(), $postItems[$modelid]));
                        $model->save();
                    } catch (\Exception $e) {
                        $messages[] = "[Tigren Blog Post ID: {$modelid}]  {$e->getMessage()}";
                        $error = true;
                    }
                }
            }
